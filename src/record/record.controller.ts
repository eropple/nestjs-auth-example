import { Controller, Get, Param, Put, Body, HttpException } from '@nestjs/common';
import { AuthnOptional, AuthzScope, Identity, IdentifiedBill } from '@eropple/nestjs-auth';
...
export class RecordController {
  ...
  async listRecords(@Identity() identity: AppIdentityBill) {
    ...
    return this._recordService.listRecords(identity instanceof IdentifiedBill ? identity.principal : null);
  }
  ...
  async getRecord(@Param("fileName") fileName: string): Promise<Record> {
    ...
    return this._recordService.getRecordByName(fileName);
  }
  ...
  async putRecord(
    ...
    if (!putRequest || !putRequest.content) {
      throw new HttpException("Bad data: request must have a 'content' field.", 400);
    }

    return this._recordService.updateRecord(identity.principal, fileName, putRequest.content);
  }
}
