...
import { AnonymousBill } from '@eropple/nestjs-auth';
import { fromPairs, cloneDeep } from 'lodash';

import { MyAppRightsTree, PrincipalType } from '../authx/identity-types';
...
export class RecordService {
  private readonly records: { [name: string]: Record };
        ...
        view: {
          right: (scopePart, request) => {
            ...
            if (record.isPublic) {
              ...
              if (identity instanceof AnonymousBill) {
                return false;
              } else {
                switch (identity.type) {
                  case PrincipalType.USER:
                    return record.usersWithReadAccess.includes(identity.principal.username);
                  case PrincipalType.INTERNAL_SERVICE:
        ...
        edit: {
          right: (scopePart, request) => {
            ...
            if (identity instanceof AnonymousBill) {
              ...
              switch (identity.type) {
                case PrincipalType.USER:
                  return record.usersWithEditAccess.includes(identity.principal.username);
                case PrincipalType.INTERNAL_SERVICE:
                  throw new Error('TODO: implement');
              }
            }
  ...
  constructor() {
    this.records = fromPairs(records.map(r => [r.name, cloneDeep(r)]));
  }
