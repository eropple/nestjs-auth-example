...
export class LoginService {
  private readonly _users: { [name: string]: User };

  constructor() {
    this._users = fromPairs(users.map(u => [u.username, cloneDeep(u)]));
  }
   ...
   */
  async login(name: string, password: string, scopes: ReadonlyArray<string>): Promise<string | null> {
    ...
    return `${name.split("").reverse().join("")}-${scopes.join("|")}`;
  }
}
